...
public class ChargeCodesDB {
	...
	public List<ChargeCodes> getChargeCodes() {
		...
		Cursor cursor = db
				.rawQuery(
						"select a.ChargeId,a.Code,a.IsStoreIn,b.State from ChargeCodes a left join Charges b on a.ChargeId=b.Id where b.IsPackCode=0",
						null);
	...
	public void updateChargeCode(String chargeId, List<String> cs) {
		db = this._ctx.openOrCreateDatabase(Constants.DATABASE_NAME,
				Constants.DATABASE_CURRENT_VERSION, null);
		db.delete(TABLE_CHARGECODES, String.format("ChargeId='%s'", chargeId),
				null);
	...
	public List<ChargeCodes> getChargeCodes2(String chargeId) {
		...
		Cursor c = db.query(TABLE_CHARGECODES, new String[] { "ChargeId",
				"Code", "IsStoreIn" },
				String.format("ChargeId='%s'", chargeId), null, null, null,
				null);
		while (c.moveToNext()) {
			ChargeCodes cc = new ChargeCodes();
			cc.setChargeId(c.getString(c.getColumnIndex("ChargeId")));
